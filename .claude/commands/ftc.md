...
3. Layer this project's conventions on top, per `CLAUDE.md` § "Component conventions" and the `component` skill: named exports for new `components/ui/*`/`components/providers/*` files, an exported `Props` type, `data-slot="component-name"`, `cn()` for class merging, path aliases, no hardcoded colors, and `"use client"` only where actually needed. Also confirm the Next.js config allows any external image domains the Figma plugin pulled in (e.g. `localhost:3845`).
